...
1. **`KingsScore`** — bibliothèque PURE : moteur de score déterministe + sélection des réglages
   ...
   ni Docker, ni HTTP).
2. **`KingsCloud.Api`** — backend ASP.NET Core 8 + PostgreSQL (EF Core/Npgsql) : API REST conforme à
   l'OpenAPI, leaderboard **anti-triche** (recalcul serveur via `KingsScore`), licences, packs signés, RGPD.
...
## Lancer le backend (API + PostgreSQL)

```bash
docker compose up -d                       # PostgreSQL 16 (valeurs DEV LOCAL ; secrets via .env)
dotnet run --project src/KingsCloud.Api    # applique migrations + seed (dev), API conforme OpenAPI sous /v1
```

Les **tests d'intégration** (`tests/KingsCloud.Api.Tests`) démarrent l'API en mémoire sur la vraie base
et couvrent l'auth, le 401, la licence, le **leaderboard anti-triche**, le classement, les packs, le RGPD.
Migrations EF : `dotnet ef migrations add <Nom> --project src/KingsCloud.Api`.
...
src/KingsCloud.Api/          backend ASP.NET Core 8 (Controllers, Data/EF, Security, Services, Packs)
docker-compose.yml           PostgreSQL 16 local
tests/KingsScore.Tests/      tests xUnit (reproductibilité, redistribution, échelle, sélection)
tests/KingsCloud.Api.Tests/  tests d'intégration API (sur la vraie base PostgreSQL)
...
- **Anti-triche (C3)** : le leaderboard recalcule le score côté serveur via `KingsScore` à partir des
  métriques brutes ; aucune valeur de score cliente n'est lue (vérifié par test d'intégration).
- **Confidentialité (C8)** : comptes/licences/jetons stockés **hachés**, aucun e-mail en clair.

## En attente de validation / livrables humains

- **Fonction de points v0 graduée** (`PointsFunctionV1`) : proposition validée comme direction, non
  figée. `expectedScoreResult` des fixtures restent illustratifs ; figés par l'équipe avec le vrai
  catalogue (C7). Scores v0 actuels : no-thermal `global=17`, high-end `global=33` ; achievable provisoire.
- **Descripteur de tweak** (`proposals/tweak-descriptor.md`) : proposé, **NON figé** tant que le Lot A
  (TweakEngine) ne l'a pas confirmé. **Catalogue** réel : livrable humain (non fourni).
- **`rawMetrics`** (leaderboard, O2) : le serveur accepte un SystemSnapshot complet (sur-ensemble sûr) ;
  le sous-ensemble exact reste à figer A/C.
- **Heuristique de tier** (budget/mid/highEnd) : provisoire, à valider (vrai mapping cloud/catalogue).
- **`offlineToleranceRemaining`** : renvoie la tolérance configurée (le « restant » exact n'est pas modélisé).
- **Signature des packs** : mécanisme HMAC de dév ; clé asymétrique de prod = acte humain (#8),
  vérification côté Cœur. **Codes d'erreur** non-200 non spécifiés par l'OpenAPI (à figer).
